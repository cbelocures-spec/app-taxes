...
async function autoLogin(browser, username, password, portalUrl) {
  ...
  // Extract CSRF token (Taxes.com.ar now requires this on the login POST) and
  // inject it into the form as a hidden "_token" input before submitting.
  ...
  if (csrfToken) {
    console.log(`[autoLogin] CSRF token found: ${csrfToken.substring(0, 10)}... (length=${csrfToken.length})`);
    ...
    console.log('[autoLogin] CSRF token injected into login form as hidden "_token" input.');
  } else {
    console.log('[autoLogin] No CSRF meta tag found on login page (csrf-token) - proceeding without it.');
  }
