...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    sftp.writeFile('/tmp/app-taxes.service', SERVICE_CONF, (err2) => {
      ...
      conn.exec("echo CesarHernan3550 | sudo -S cp /tmp/app-taxes.service /etc/systemd/system/app-taxes.service && sudo systemctl daemon-reload && sudo systemctl restart app-taxes.service", (err3, stream) => {
        let out = '';
        stream.on('data', d => out += d);
        stream.on('close', () => {
          console.log("=== APP-TAXES SERVICE UPDATED AND RESTARTED ===");
          console.log(out);
          conn.end();
        });
      });
