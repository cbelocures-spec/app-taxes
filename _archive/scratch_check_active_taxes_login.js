...
(async () => {
  try {
    console.log("Starting Puppeteer test login on 192.168.50.4...");
    ...
    const passwordsToTest = ['Paniol2015', 'paniol2015', 'Paniol2024', 'Paniol2025', '123456'];
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    sftp.writeFile('/home/cbelocures/gestion/test_active_login.js', TEST_SCRIPT, (err2) => {
      if (err2) return conn.end();
      conn.exec("cd /home/cbelocures/gestion && node test_active_login.js 2>&1", (err3, stream) => {
        let out = '';
        stream.on('data', d => out += d);
        stream.on('close', () => {
          console.log("=== TAXES LOGIN TEST OUTPUT ===");
          console.log(out);
          conn.end();
        });
      });
