...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    sftp.writeFile('/home/cbelocures/gestion/test_debug_core.js', TEST_SCRIPT, (err2) => {
      if (err2) return conn.end();
      conn.exec("node /home/cbelocures/gestion/test_debug_core.js 2>&1", (err3, stream) => {
        let out = '';
        stream.on('data', d => out += d);
        stream.on('close', () => {
          console.log("=== TAXES LOGIN TEST RESULT FROM GESTION DIR ===");
          console.log(out);
          conn.end();
        });
      });
