...
const final3Tasks155 = [
  {
    id: "task-155-ojeda-final",
    centroCosto: "15",
...
const final2Tasks119 = [
  {
    id: "task-119-morel-final",
    centroCosto: "15",
...
async function run() {
  ...
  // Deduplicate tasks: keep only finished historical tasks + 1 clean set of pending tasks for 155 and 119
  for (const o of orders) {
    let modified = false;

    if (String(o.interno) === '155' && !o.archived && !o.deleted) {
      const historical = (o.tasks || []).filter(t => t.status === 'Finalizada');
      o.tasks = [...historical, ...final3Tasks155];
      modified = true;
    } else if (String(o.interno) === '119' && !o.archived && !o.deleted) {
      const historical = (o.tasks || []).filter(t => t.status === 'Finalizada');
      o.tasks = [...historical, ...final2Tasks119];
      modified = true;
    }
  ...
  console.log("\n=== FINAL VERIFICATION OF EXACT 5 ACTIVE PENDING TASKS ===");
  let count = 0;
  ...
  console.log(`\nTOTAL UNIQUE ACTIVE PENDING TASKS IN SYSTEM: ${count}`);

  // Write to local db.json and upload to Debian persistent db.json
  ...
  conn.on('ready', () => {
    conn.sftp((err, sftp) => {
      ...
      const tmp = './db_exact_5_final_clean.json';
