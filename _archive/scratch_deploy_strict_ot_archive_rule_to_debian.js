  ...
  conn.on('ready', () => {
    conn.exec('cat /home/cbelocures/data/db.json', async (err, stream) => {
      ...
      stream.on('close', async () => {
        ...
        // Enforce strict unarchiving for orders missing taxesOrderNumber
        (db.workOrders || []).forEach(o => {
          if (!o.taxesOrderNumber || String(o.taxesOrderNumber).trim() === '') {
            o.archived = false;
            o.archivedAt = null;
            pushOrderToRailway(o.id, { ...o, archived: false });
          }
        });
        ...
        conn.sftp(async (sftpErr, sftp) => {
          ...
          const tmp = './db_strict_ot_fixed.json';
