...
async function run() {
  ...
  console.log("=== FINAL VERIFICATION OF THE 5 ACTIVE PENDING TASKS ===");
  let total = 0;
  finalOrders.forEach(o => {
    (o.tasks || []).forEach(t => {
      if (t.status === 'Pendiente' && !o.archived && !o.deleted) {
        total++;
        console.log(`Task #${total} (OT Interno ${o.interno}): ${t.empleado} -> ${t.descripcion}`);
      }
    });
  });
  console.log(`TOTAL ACTIVE PENDING TASKS: ${total}`);

  // Sync db.json to local and Debian
  ...
  conn.on('ready', () => {
    conn.sftp((err, sftp) => {
      ...
      const tmp = './db_5tasks_final.json';
