...
if (fs.existsSync(dbFile)) {
  ...
  if (o155) {
    // Keep all existing tasks, but append/ensure the 3 target tasks are present in 'Pendiente' status
    const tasks155 = [
      {
        id: `task-155-target-1-${Date.now()}`,
        centroCosto: "15",
    ...
    // Set other tasks of 155 to Finalizada
    (o155.tasks || []).forEach(t => t.status = 'Finalizada');
    o155.tasks = [...(o155.tasks || []), ...tasks155];
    console.log("✅ Created 3 target tasks for Interno 155!");
  }
  ...
  if (o119) {
    ...
    // Set other tasks of 119 to Finalizada
    (o119.tasks || []).forEach(t => t.status = 'Finalizada');
    o119.tasks = [...(o119.tasks || []), ...tasks119];
    console.log("✅ Created 2 target tasks for Interno 119!");
  }
...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut('./db.json', '/home/cbelocures/data/db.json', (err1) => {
      console.log("✅ Uploaded db.json with exact 5 tasks to 192.168.50.4!");
