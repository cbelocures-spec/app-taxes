...
async function main() {
  ...
  // Deduplicate tasks per order based on description + employee
  for (const o of orders) {
    if (!o.tasks || !Array.isArray(o.tasks)) continue;
    ...
    if (cleanTasks.length !== o.tasks.length) {
      o.tasks = cleanTasks;
      console.log(`Cleaned dupes for Order ${o.id} (Interno ${o.interno}) -> ${cleanTasks.length} tasks remaining.`);
  ...
  console.log("\n=== FINAL VERIFIED DEDUPLICATED ACTIVE PENDING TASKS ===");
  let count = 0;
  ...
  // Write clean DB to local db.json and upload to Debian persistent DB /home/cbelocures/data/db.json
