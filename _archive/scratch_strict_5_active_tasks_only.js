...
const clean3Tasks155 = [
  {
    id: "task-155-ojeda-clean-target",
    centroCosto: "15",
  ...
  {
    id: "task-155-perez-clean-target",
    centroCosto: "15",
  ...
  {
    id: "task-155-rocha-clean-target",
    centroCosto: "15",
...
const clean2Tasks119 = [
  {
    id: "task-119-morel-clean-target",
    centroCosto: "15",
  ...
  {
    id: "task-119-canaviri-clean-target",
    centroCosto: "15",
...
async function run() {
  ...
  // Process all orders: ONLY Order 1784884919356 gets the 3 pending tasks for 155, ONLY Order 1785334094327 gets the 2 pending tasks for 119
  // All other 155/119 orders have their tasks finalized so they don't produce ghost duplicates.
  for (const o of orders) {
    let modified = false;

    if (String(o.id) === '1784884919356') {
      const historical = (o.tasks || []).filter(t => t.status === 'Finalizada' && !t.id.includes('clean-target'));
      o.tasks = [...historical, ...clean3Tasks155];
      o.archived = false;
      o.deleted = false;
      modified = true;
    } else if (String(o.interno) === '155') {
    ...
    if (String(o.id) === '1785334094327') {
      const historical = (o.tasks || []).filter(t => t.status === 'Finalizada' && !t.id.includes('clean-target'));
      o.tasks = [...historical, ...clean2Tasks119];
      o.archived = false;
      o.deleted = false;
      modified = true;
    } else if (String(o.interno) === '119') {
      (o.tasks || []).forEach(t => { if (t.status === 'Pendiente') t.status = 'Finalizada'; });
      modified = true;
    }
  ...
  console.log("=== FINAL STRICT VERIFICATION: EXACTLY 5 ACTIVE PENDING TASKS ===");
  let pendingCount = 0;
  ...
  // Push to local db.json and upload to Debian persistent DB /home/cbelocures/data/db.json
  ...
  conn.on('ready', () => {
    conn.sftp((err, sftp) => {
      ...
      const tmp = './db_strict_5_clean.json';
