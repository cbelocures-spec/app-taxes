...
conn.on('ready', () => {
  conn.exec('cat /home/cbelocures/gestion/db.json', (err, stream) => {
    let out = '';
    stream.on('close', () => {
      try {
        const db = JSON.parse(out);
        console.log("=== DEBIAN DIRECT GESTION DB VERIFICATION ===");
        let count = 0;
        ...
        console.log(`TOTAL ACTIVE TASKS ON DEBIAN GESTION DB: ${count}`);
