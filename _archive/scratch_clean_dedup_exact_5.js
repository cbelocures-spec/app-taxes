...
async function main() {
  ...
  if (o155) {
    ...
    o155.archived = false;
    o155.deleted = false;
    await putOrderWithRetry(o155.id, o155);
  }
  ...
  const finalOrders = await getRailwayOrders();

  console.log("=== FINAL VERIFICATION: EXACT 5 ACTIVE TASKS ===");
  let pendingCount = 0;
    ...
    if (!o.archived && !o.deleted) {
      (o.tasks || []).forEach(t => {
        if (t.status === 'Pendiente' || !t.status) {
          pendingCount++;
          console.log(`[#${pendingCount}] Interno ${o.interno} (${o.rodado}): [${t.empleado}] ${t.descripcion}`);
        }
      });
    }
  });
  ...
  // Sync db.json to local and Debian persistent DB
  ...
  conn.on('ready', () => {
    conn.sftp((err, sftp) => {
      ...
      const tmp = './db_exact_5_final_sync.json';
