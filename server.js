...
app.post('/api/orders', (req, res) => {
  try {
    const { rodado, responsable, fechaEntrega, horario, interno, clasificacion, incidente, tasks, estadoUnidad, combustibleReset, sector: sectorFromClient, area, pendingElastiquero, lavadoParticularPersona } = req.body;
    ...
    const newOrder = db.createWorkOrder({
      ...
      lavadoParticularPersona: lavadoParticularPersona ? String(lavadoParticularPersona).trim() : null
    });
...
app.get('/api/tipos-lavado', (req, res) => {
  try {
    res.json({ tipos: db.getTiposLavado() });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post('/api/tipos-lavado', (req, res) => {
  try {
    const { label, descripcion, categoria } = req.body;
    const item = db.addTipoLavado({ label, descripcion, categoria });
...
app.patch('/api/tipos-lavado/:key', (req, res) => {
  try {
    const { label, descripcion, categoria } = req.body;
    const item = db.updateTipoLavado(req.params.key, { label, descripcion, categoria });
    res.json({ tipo: item });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
app.get('/api/personas-lavado-ap', (req, res) => {
  try {
    res.json({ personas: db.getPersonasLavadoAP() });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post('/api/personas-lavado-ap', (req, res) => {
  try {
...
app.get('/api/sectores-playa', (req, res) => {
  try {
    res.json({ sectores: db.getSectoresPlaya() });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post('/api/sectores-playa', (req, res) => {
  try {
    const sectores = db.addSectorPlaya(req.body && req.body.nombre);
    res.json({ sectores });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
